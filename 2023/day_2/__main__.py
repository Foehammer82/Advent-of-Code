...
def game_is_valid(game_info: str) -> bool:
    for game_round in game_info.split(":")[1].split(";"):
        cubes_pulled = {
            balls.strip().split(" ")[1]: balls.strip().split(" ")[0]
            for balls in game_round.split(",")
        }

        for color, count in cubes_pulled.items():
            if int(count) > available_cubes[color]:
                return False

    return True


def get_the_minimum_game_power(game_info: str) -> int:
    minimum_cubes = {}
    ...
    for game_round in game_info.split(":")[1].split(";"):
        ...
        for color, count in cubes_pulled.items():
            if color not in minimum_cubes or int(count) > minimum_cubes[color]:
                minimum_cubes[color] = int(count)

    result = 1
    for count in minimum_cubes.values():
        result *= count
    return result


if __name__ == "__main__":
    # Part 1 - Determine which games would have been possible and sum their IDs
    assert [game_is_valid(game_info) for game_info in test_games] == [
        True,
        True,
        False,
        False,
    ...
    with open("puzzle_input") as f:
        for line in f.readlines():
            if game_is_valid(line):
                sum_of_valid_games += int(line.split(":")[0].split(" ")[1])

    print(f"Sum of valid games: {sum_of_valid_games}")

    # Part 2 - Determine the minimum set of cubes that must have been present and sum their powers
    assert [get_the_minimum_game_power(game_info) for game_info in test_games] == [
        48,
        12,
        1560,
        630,
    ...
    sum_of_minimum_game_powers = 0
    with open("puzzle_input") as f:
        for line in f.readlines():
            sum_of_minimum_game_powers += get_the_minimum_game_power(line)

    print(f"Sum of minimum game powers: {sum_of_minimum_game_powers}")
