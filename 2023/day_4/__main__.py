...
winning numbers! That means card 1 is worth 8 points (1 for the first match, then doubled three times for
...
Once all the originals and copies have been processed, you end up with 1 instance of card 1, 2 instances of
card 2, 4 instances of card 3, 8 instances of card 4, 14 instances of card 5, and 1 instance of card 6. In total, this
...
def get_card_matches(card: str) -> int:
    winning_raw, actual_raw = tuple(card.split(":")[1].strip().split("|"))
    winning = set(winning_raw.strip().split())
    actual = set(actual_raw.strip().split())
    return len(winning.intersection(actual))


def get_points_given_matches(matches: int) -> int:
    return 2 ** (matches - 1) if matches > 0 else 0


def get_card_points(card: str) -> int:
    matches = get_card_matches(card)
    points = get_points_given_matches(matches)
    return points


def get_all_card_matches(card_list: list[str]) -> dict[int, int]:
    card_matches = {}
    for i, card in enumerate(card_list, start=1):
        card_matches[i] = get_card_matches(card)

    return card_matches


def get_total_cards(card_matches: dict[int, int]) -> int:
    card_info_dict = {
        card_id: [
            matches,
            1,
    ...
    for card_id, card_info in card_info_dict.items():
        for i in range(1, card_info[0] + 1):
            card_info_dict[card_id + i][1] += card_info_dict[card_id][1]

    return sum([v[1] for v in card_info_dict.values()])


if __name__ == "__main__":
    # Part 1
    assert [get_card_points(card) for card in test_input.splitlines()] == [
        8,
        2,
        2,
        1,
    ...
    with open("puzzle_input") as puzzle_input:
        part_1_final_answer = sum(get_card_points(card) for card in puzzle_input)

    print(f"Total points in part 1: {part_1_final_answer}")

    # Part 2
    test_card_matches = get_all_card_matches(test_input.splitlines())
    assert get_total_cards(test_card_matches) == 30

    with open("puzzle_input") as puzzle_input:
        card_matches = {
            i: get_card_matches(k) for i, k in enumerate(puzzle_input, start=1)
        }
    total_cards = get_total_cards(card_matches)

    print(f"Total cards in part 2: {total_cards}")
